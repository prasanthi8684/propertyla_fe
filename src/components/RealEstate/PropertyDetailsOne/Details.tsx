...
import {
  ActiveWishListSvg, BathroomsSvg, BedroomsSvg,
  CartSvg, LivingSvg, WishListSvg,
} from "@/components/SVG";
...
function mapToDisplay(item: ApiProperty): IFeaturedPropertyDT {
  const address = [item.streetName, item.cityName, item.state].filter(Boolean).join(", ");
  ...
  const livingArea = area > 0
    ? (Number.isInteger(area) ? `${area} Sq Ft` : `${area.toFixed(1)} Sq Ft`)
    : "N/A";
  return {
    id: 0,
            ...
            <div className="col-lg-8">
              <div className="tp-property-details-heading mb-40">
                <div className="mb-2 d-flex gap-2 flex-wrap">
                  <span style={{
                    background: apiProperty.listingType === "rent" ? "#e8f4fd" : "#eefbee",
                    color:      apiProperty.listingType === "rent" ? "#1a73e8" : "#2e7d32",
                ...
                <span style={{ color: "#777", fontSize: "15px" }}>{display.address}</span>

                <div className="tp-property-details-info mt-3">
                  <span><BedroomsSvg /> {display.bedrooms} Bed</span>
                  <span style={{ margin: "0 12px" }}><BathroomsSvg /> {display.bathrooms} Bath</span>
                  <span><LivingSvg /> {display.livingArea}</span>
            ...
            <div className="col-lg-4">
              <div className="tp-property-details-right-side text-lg-end mb-40">
                <div className="tp-property-details-icon-box mb-3">
                  <button title="Bookmark"><span><BookmarkSvg /></span></button>
                  <button title="Compare" onClick={() => dispatch(compire_product(display))}>
                    <span><CompireSvgTwo /></span>
                  </button>
                  <button title="Wishlist" onClick={() => dispatch(toggle_wishlist(display))}>
                    <span>{isWishlisted
                      ? <ActiveWishListSvg width="26" height="26" />
                      : <WishListSvg width="20" height="20" />}
                    </span>
                  </button>
                  <button title="Add to cart" onClick={() => dispatch(cart_product(display))}>
                ...
                {apiProperty.negotiable && (
                  <p style={{ color: "#2e7d32", fontSize: "13px", fontWeight: 500, marginTop: "6px" }}>
                    ✓ Negotiable
                  </p>
                )}
